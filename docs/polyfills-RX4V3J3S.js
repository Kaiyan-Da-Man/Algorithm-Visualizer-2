	...
	function z(f, u, i) {
		...
		if (f[q] === M) {
			...
			if (
				...
				for (let g = 0; g < w.length; )
					ee(f, w[g++], w[g++], w[g++], w[g++]);
				if (w.length == 0 && u == x) {
					f[q] = d;
					let g = i;
					try {
	...
	class t {
		...
		then(u, i) {
			...
			let m = new h(se),
				w = n.current;
			return (
				this[q] == M ? this[R].push(w, m, u, i) : ee(this, w, m, u, i),
				m
			);
		}
		catch(u) {
			...
			let m = n.current;
			return (
				this[q] == M ? this[R].push(m, h, u, u) : ee(this, m, h, u, u),
				h
			);
		}
	}
					...
					try {
						if (O.hasOwnProperty(p)) {
							...
							Z && Z.value
								...
								: O[p] &&
									(O[p] = e.wrapWithCurrentZone(O[p], I));
